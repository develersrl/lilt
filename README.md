...
The development settings are stored inside `config.dev.json` file under the `app` directory.
Production settings must be stored inside a `config.prod.json` file in the same directory.
The production settings file is git-ignored.


### [Android environment setup](#index)
...
`gradle.properties` file (usually located in `./gradle` directory of the home folder).
